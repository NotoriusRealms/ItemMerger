...
public final class SecondSubcommand implements Subcommand {

    private final PluginManager manager;

    public SecondSubcommand(PluginManager manager) {
        this.manager = manager;
    }

    @Override
    public String getPermission() {
        return "itemmerger.second";
    }
    ...
    public void perform(Player player, String[] args) {
        ...
        manager.getRecipeManager().setSecond(recipe, item);

        player.sendMessage(SECOND_ITEM.getMessage().replaceAll("%recipe_name%", recipe.getName()));
    }
}
